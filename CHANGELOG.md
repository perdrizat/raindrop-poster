...
### Fixed
- Queue page loaded forever ("Loading articles...") when the app was opened from an origin without localStorage settings (e.g. behind a reverse proxy while Setup ran against the direct IP): workflow settings (`selectedTag`, `postingObjectives`) now flow from the server (`/api/system/status`, SQLite) into PostPage instead of per-origin localStorage — this also fixes proposals silently using a generic prompt and published bookmarks never being retagged `_posted` on the second origin. localStorage remains a fallback when the server has no value.
- Empty `selectedTag` now shows an explicit "No Tag Selected" state with a link to Setup instead of an infinite loading spinner.
- Fresh origins land on the Queue (not Setup) when the server already has a configured tag.
- Server unit tests no longer write into the developer's `server/raindrop.sqlite` (they had blanked a real API key): vitest now points `DATA_DIR` at a temp dir, guarded by `db-isolation.test.js`.
- "Built:" tooltip now shows the viewer's local time: the build stamp is baked as ISO-8601 UTC and formatted with `toLocaleString()` at render time, instead of displaying an unlabeled build-host wall-clock string that read like UTC.
- "Built:" badge no longer goes stale on server-only rebuilds: `VITE_BUILD_TIME` was stamped inside a cached Docker layer, so a rebuild that didn't touch client files shipped the old timestamp (deploy looked like it hadn't landed). Now passed as a `--build-arg` evaluated per build; only the stamp + Vite compile re-run on a cache-warm build, and the stamp file is written with a single overwrite (no append).
- Venice blank-image root cause: blanks came back with the request echoing `steps:0` (no denoising), while good renders ran ~39s — the image endpoint now sends an explicit non-zero `steps` so Venice can't default to a blank render, and the success log prints the resolved step count to confirm/refute the fix. Retry loop kept as a safety net.
...
### Changed
- `pnpm deploy:remote` now redeploys the TrueNAS app automatically after streaming the image (`midclt call app.redeploy`; app name via `DEPLOY_APP_NAME`, default `raindrop-poster`) — no more manual UI restart, which was needed because a running container is pinned to an image ID, not a tag.
